...
const SAMPLE_AI_ANSWERS = {
  design: {
    focalPoint: rate("no", "three blocks compete at equal weight"),
    alignment: rate("yes", "content sits on one consistent column"),
    sectionBoundaries: rate("no", "sections run together, no dividers"),
    breathingRoom: rate("no", "text runs to the page edges"),
    spacingConsistency: rate("yes", "section gaps look evenly sized"),
    noDeadSpace: rate("yes", "no large unexplained empty regions"),
    fontRestraint: rate("no", "four typefaces visible on one page"),
    headingContrast: rate("yes", "headings clearly larger than body"),
    lineLength: rate("no", "paragraphs stretch the full window width"),
    paletteRestraint: rate("yes", "cream, brown and one accent only"),
    textContrast: rate("no", "light grey caption text on white"),
    ...
    imageRelevance: rate("yes", "photos show the actual bread and shop"),
  },
  trust: {
    customLogo: rate("yes", "hand-drawn wordmark in the header"),
    distinctBranding: rate("no", "layout is an unmodified theme"),
    ...
    expectationsSet: rate("no", "no delivery or response times given"),
    offerSpecific: rate("yes", "names two-day prove and stone oven"),
    noDarkPatterns: rate("yes", "no countdowns or forced urgency"),
    realPhotography: rate("yes", "photos show the real bakers"),
    contentFreshness: rate("no", "no dates or recent references"),
  },
  ux: {
    ...
    valueInFiveSeconds: rate("no", "opening text describes process, not offer"),
    headlineStatesValue: rate("no", "headline is the business name only"),
    audienceIdentifiable: rate("yes", "local Fitzroy shoppers and cafes"),
    bodyTextSize: rate("yes", "body copy is comfortably sized"),
